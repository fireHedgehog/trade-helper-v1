...
- [ ] Point-in-time equity membership/delisting data — unlocks cross-sectional equity momentum, value, quality, broad-universe factor work; the ~500-symbol universe is a today's-snapshot, survivorship-biased. Shared blocker behind 6 [cross-sectional idea library](2026-08-20-cross-sectional-experiment-ideas.md) candidates, not one — high-leverage if cleared. Engine feasibility already checked (`0.61.0`, `N=495`, zero code change) — see [result](../research-results/cross-sectional-equity-momentum-feasibility-v1.md); only the data purchase itself remains unpurchased.
