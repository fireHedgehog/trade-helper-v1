...
- [x] Point-in-time equity membership/delisting data — turned out not to need a vendor purchase at all: [`universe_pit.py`](../../backend/app/universe_pit.py) (`0.81.0`) ingests a free, MIT-licensed, hand-maintained S&P 500 membership history ([fja05680/sp500](https://github.com/fja05680/sp500)), live-verified (`1,259` intervals, `1,206` symbols, `503` currently active). Shared blocker behind 6 [cross-sectional idea library](2026-08-20-cross-sectional-experiment-ideas.md) candidates, now cleared for the S&P 500 subset — engine feasibility was already checked (`0.61.0`, `N=495`, zero code change) — see [result](../research-results/cross-sectional-equity-momentum-feasibility-v1.md). Ingestion existing still authorizes nothing: CS-01's re-baseline still needs its own hypothesis-engineering note, Stage 9A score, and preregistration before a real run.
...
- [ ] Options-implied volatility — forward-looking risk measure and market-implied macro expectation, neither available today.
