...
| Active research | None active. The single-asset/time-series research line is formally **parked** (not rejected) — see [stage-closures/2026-08-20-single-asset-time-series-line.md](stage-closures/2026-08-20-single-asset-time-series-line.md) for the full evidence inventory, lessons, and gate condition. The next line is explicitly undecided: cross-sectional/relative-performance work (Fed put and the wider [macro reaction-function narrative library](brainstorm/2026-08-20-macro-reaction-function-narrative-library.md), or a larger cross-sectional-momentum extension still blocked on Tier 4 paid point-in-time equity data) is a live candidate, not a default — it must clear the same scored evaluation CTA v2 did. `app.macro_pit` (`0.60.0`) closes the macro line's engineering blocker (ADR 0006 clauses 2–4) but authorizes nothing by itself: no live ingestion has run (needs a free, self-registered `FRED_API_KEY` nobody has obtained yet), and every macro candidate still needs its own hypothesis-engineering → Stage 9A score → preregistration before any signal use; Stage 9B remains gated |
