...
| Current state | Research reorganized into a chaptered, living program — see [research-program.md](research-program.md). Chapters 1-3 hold the existing strict falsification standard unchanged; Chapter 4 (risk-budgeted ensemble construction, [ADR 0007](adr/0007-risk-budgeted-ensemble-acceptance.md), accepted) answers "is this worth a small, sized bet," not "is this proven," and now has a real breadth generator (§5, the 17-formula factor zoo) instead of one hand-picked candidate at a time; Chapter 5 (operational bridge, [ADR 0008](adr/0008-bounded-paper-trading.md), **accepted** `2026-08-21`) answers a third question — what has to be true operationally (point-in-time data, sizing, reconciliation, approval gate) before any candidate that does clear a ladder can actually be paper-traded, via three approval paths (strict ladder, Chapter 4, or Track B — a lighter, statistically-unproven-but-risk-disciplined entry for well-known patterns). Build-vs-buy resolved toward using a broker's own free paper-trading API (Alpaca) rather than a custom fill simulator. Acceptance settled the Chapter 5 design; none of its required infrastructure is built yet |
| Verification | `384 passed` at `0.74.0` |
| Completed | Stage 8; Stage 9A Cycles 1–6; CTA v1/v2; TA Breakout, Wave Pull, ETF-12 rotation, both calendar candidates, Overnight Gap, Fed put yield-stress precursor v1+v2+v3 — all closed and indexed in [research-protocols/README.md](research-protocols/README.md); labor-market claims lead-lag operationalized and bounded-explored (not yet scored); Chapter 4's first full pass (CTA v2, Wave Pull, Calendar Day-of-Week) closed with a correlation-aware significance test, not an approximation; ADR 0008 accepted `2026-08-21` (design only, not implemented); factor zoo v1 (17 WorldQuant-style formulas) scanned against the real 495-symbol universe with real Sharpe/IC and real charts — screening only, no eligibility conferred |
| Next product work | Two independent threads. (1) Factor zoo follow-up: propose the least-redundant screen survivors (`alpha028`/`004`/`026`) as individual Chapter 4 candidates with a stated mechanism each, and run a transaction-cost-sensitivity check on the reversal cluster (`alpha034`/`033`/`009`) before trusting its Sharpe numbers — see [research-program.md](research-program.md) Chapter 4 §5. (2) ADR 0008 implementation — `live_price_snapshots`, `paper_ledger_events`, `research_catalog.py` corrections, and the Alpaca integration module (`backend/app/paper_broker.py`) are all still unbuilt, and real connectivity is blocked on the user creating an Alpaca paper account and API keys; schema/metrics work can proceed without that. No queued Stage 9A candidate, for the same underlying reason as Active research's Chapter 4 status. Chapter 1-3 open threads (not yet sections): Dow theory swing structure, Fibonacci, golden/death cross, 52-week-high anchoring (Ch.1); the six Tier-4-blocked cross-sectional ideas (Ch.2); the 7-variable macro library, PEAD/earnings-gap, policy-exposure factor (Ch.3) — see [research-program.md](research-program.md) for the full inventory |
| Active research | Chapter 4 (ADR 0007, accepted): 3 hand-picked candidates scored, none currently has a settled, adversarially-checked positive read. CTA v2 cleanly not eligible. Wave Pull's `TLT`-only "eligible" report was a winner's-curse artifact — symmetric 12-asset rescoring (`2/11`, `GLD`+`TLT`) is statistically indistinguishable from the calibrated chance rate (`P(X≥2)≈65%`); walked back to "clean candidate for Chapters 1-3," not "Chapter 4-eligible." Calendar Day-of-Week's `6/12` was directly tested with a correlation-aware joint null (not an approximation) and is also not distinguishable from chance (full numbers: CHANGELOG `0.71.0`, [research-program.md](research-program.md) §4). Orthogonality measured across all 8 nominally-eligible signal-slots (3 of 28 pairs redundant, all 3 among Day-of-Week's winners) and the full 12-asset Day-of-Week correlation matrix (`31`/`66` pairs redundant). §5 (new): the factor zoo screen found a real top cluster (`alpha034`/`033`/`028`/`004`, Sharpe `0.47`-`0.76`) but flagged it as a probable bid-ask-bounce reversal artifact pending a cost-sensitivity check — not yet proposed as formal candidates. Stage 9B remains gated. Chapter 5 (ADR 0008, accepted): zero strategies or candidates currently hold `eligible for operational validation` under any of the three approval paths — implementation not yet started |
