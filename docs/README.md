...
| Verification | `289 passed` at `0.56.0` (calibration study is a standalone diagnostic script, not part of the pytest suite); `0.58.0`/`0.59.0` are docs-only, no code touched |
...
| Active research | None active. The single-asset/time-series research line is formally **parked** (not rejected) — see [stage-closures/2026-08-20-single-asset-time-series-line.md](stage-closures/2026-08-20-single-asset-time-series-line.md) for the full evidence inventory, lessons, and gate condition. The next line is explicitly undecided: cross-sectional/relative-performance work (Fed put, or a larger cross-sectional-momentum extension currently blocked on Tier 4 point-in-time universe data) is a live candidate, not a default — it must clear the same scored evaluation CTA v2 did; Stage 9B remains gated |
