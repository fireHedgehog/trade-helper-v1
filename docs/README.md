...
| Current state | Research reorganized into a chaptered, living program — see [research-program.md](research-program.md). Chapters 1-3 hold the existing strict falsification standard unchanged; Chapter 4 (risk-budgeted ensemble construction, [ADR 0007](adr/0007-risk-budgeted-ensemble-acceptance.md), accepted) answers "is this worth a small, sized bet," not "is this proven"; new Chapter 5 (operational bridge, [ADR 0008](adr/0008-bounded-paper-trading.md), **proposed**, not yet accepted) answers a third question — what has to be true operationally (point-in-time data, sizing, reconciliation, approval gate) before any candidate that does clear a ladder can actually be paper-traded, now via three approval paths (strict ladder, Chapter 4, or Track B — a lighter, statistically-unproven-but-risk-disciplined entry for well-known patterns). Build-vs-buy resolved toward using a broker's own free paper-trading API (Alpaca) rather than a custom fill simulator |
| Verification | `367 passed` at `0.72.0` |
| Completed | Stage 8; Stage 9A Cycles 1–6; CTA v1/v2; TA Breakout, Wave Pull, ETF-12 rotation, both calendar candidates, Overnight Gap, Fed put yield-stress precursor v1+v2+v3 — all closed and indexed in [research-protocols/README.md](research-protocols/README.md); labor-market claims lead-lag operationalized and bounded-explored (not yet scored); Chapter 4's first full pass (CTA v2, Wave Pull, Calendar Day-of-Week) is now closed with a correlation-aware significance test, not an approximation; ADR 0008 drafted (design only, not implemented) |
| Next product work | ADR 0008 needs explicit review/acceptance before any implementation begins (same sequencing ADR 0007 followed) — no new tables, no `research_catalog.py` edits, no broker integration code exist yet, by design. No queued Stage 9A candidate either, for the same underlying reason as Active research's Chapter 4 status. Chapter 1-3 open threads (not yet sections): Dow theory swing structure, Fibonacci, golden/death cross, 52-week-high anchoring (Ch.1); the six Tier-4-blocked cross-sectional ideas (Ch.2); the 7-variable macro library, PEAD/earnings-gap, policy-exposure factor (Ch.3) — see [research-program.md](research-program.md) for the full inventory |
| Active research | Chapter 4 (ADR 0007, accepted): 3 candidates scored, none currently has a settled, adversarially-checked positive read. CTA v2 cleanly not eligible. Wave Pull's `TLT`-only "eligible" report was a winner's-curse artifact — symmetric 12-asset rescoring (`2/11`, `GLD`+`TLT`) is statistically indistinguishable from the calibrated chance rate (`P(X≥2)≈65%`); walked back to "clean candidate for Chapters 1-3," not "Chapter 4-eligible." Calendar Day-of-Week's `6/12` was directly tested with a correlation-aware joint null (not an approximation) and is also not distinguishable from chance (full numbers: CHANGELOG `0.71.0`, [research-program.md](research-program.md) §4). Orthogonality measured across all 8 nominally-eligible signal-slots (3 of 28 pairs redundant, all 3 among Day-of-Week's winners) and the full 12-asset Day-of-Week correlation matrix (`31`/`66` pairs redundant). Stage 9B remains gated. Chapter 5 (ADR 0008, proposed, now including Track B): zero strategies or candidates currently hold `eligible for operational validation` under any of the three approval paths |
...
- Cron, live trading, live broker connectivity, and AWS remain out of scope. Paper trading's design is drafted ([ADR 0008](adr/0008-bounded-paper-trading.md), `proposed`) but not accepted, implemented, or authorized for use.
...
| What has to be true operationally before any candidate can be paper-traded? | [ADR 0008 (proposed)](adr/0008-bounded-paper-trading.md) |
