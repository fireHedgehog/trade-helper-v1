...
| Current state | [Chaptered research program](research-program.md). **Chapter 2 §3 closed**: cross-sectional momentum v1 (CS-01), `not_material_or_not_consistent` — see [result](research-results/cross-sectional-momentum-v1.md). GICS sector data layer built ([`universe_sectors.py`](../backend/app/universe_sectors.py)), unblocking CS-07/CS-08 (sector-vs-sector rotation, e.g. "semis vs. software") — a real, separate, deeper survivorship bias was also named and disclosed (index membership is itself endogenous to size/performance; a name can leave an index by reconstitution while still trading, invisible to any index-membership-based universe — likely a real Tier 4 vendor-data case if ever pursued, see [the idea library](brainstorm/2026-08-20-cross-sectional-experiment-ideas.md)). **[ADR 0010 (accepted)](adr/0010-long-short-ensemble-construction.md)**: amends ADR 0004's account model, designs Chapter 4's ensemble-construction engine (alpha model, risk model, optimizer) — engine (§3b/§3c) not yet built. Chapter 4 (ADR 0007) remains the primary actionable line — 2 live candidates, `atr_normalized` (Tier A execution, "ATR Vol Premium") and `amihud_illiquidity` (next for the same treatment). Chapters 1/3 deliberately paused. Chapter 5 ([ADR 0008](adr/0008-bounded-paper-trading.md)) accepted, not built. Product: Today/Symbol Research/Strategy Lab/Strategy Management unified across Tier A/B ([strategy-library.md](strategy-library.md)); Macro page carries a labeled, untested "equity read" hypothesis per event ([ADR 0006](adr/0006-macro-data-contract.md) clause 2 exception); sitewide footer links the chaptered research program. |
| Verification | `432 passed` at `0.83.0` |
| Next product work | (1) **Chapter 2, sector rotation**: hypothesis-engineering note + Stage 9A score + preregistration for a GICS-sector-vs-sector relative-strength test (CS-07/CS-08 shape) — data layer ready, experiment not started. (2) **ADR 0010**: build the risk model (§3b, shrinkage covariance) and optimizer (§3c, rank-and-weight heuristic) — real, new engineering, not yet started. (3) **Chapter 2**: CS-02/03/04/05/09 (non-sector ideas), each needs its own hypothesis-engineering note, Stage 9A score, and preregistration. (4) `amihud_illiquidity`: Tier A cross-sectional translation (per-symbol "in today's top-illiquidity-quintile" flag, same shape as `AtrVolPremium`). (5) `atr_normalized` + `amihud_illiquidity`: mechanism + confidence-multiplier scoring (Chapter 4 clauses 1/2) before a formal proposal. (6) `factor-zoo-v1` still deferred from Strategy Management, pending a clearer explanation. (7) Earnings-dates or SEC EDGAR fundamentals data layer — neither started, [data-layers.md](data-layers.md). (8) ADR 0008 implementation — blocked on an Alpaca account. |
...
| Can a Chapter 4 ensemble be long-short, and how is it constructed (alpha model, risk model, optimizer)? | [ADR 0010 (accepted)](adr/0010-long-short-ensemble-construction.md) |
