...
| Current state | [Chaptered research program](research-program.md). **Chapter 2 (cross-sectional) is now unblocked**: [`universe_pit.py`](../backend/app/universe_pit.py) ingests a free, point-in-time S&P 500 membership history, clearing the Tier 4 blocker behind 6 queued ideas (CS-01/02/03/04/05/09) — a re-baselined CS-01 momentum test at real breadth (not the prior `N=12` ETF null) is the natural next section. Chapter 4 (ADR 0007) remains the primary actionable line — 2 live candidates, `atr_normalized` (has a Tier A execution, "ATR Vol Premium") and `amihud_illiquidity` (next for the same treatment). Chapters 1/3 deliberately paused. Chapter 5 ([ADR 0008](adr/0008-bounded-paper-trading.md)) accepted, not built. Product: Today/Symbol Research/Strategy Lab/Strategy Management unified across Tier A/B ([strategy-library.md](strategy-library.md)); Macro page carries a labeled, untested "equity read" hypothesis per event ([ADR 0006](adr/0006-macro-data-contract.md) clause 2 exception); sitewide footer links the chaptered research program. |
| Verification | `422 passed` at `0.81.0` |
| Next product work | (1) **Chapter 2**: hypothesis-engineering note + Stage 9A score + preregistration for CS-01 (momentum re-baseline, real S&P 500 breadth via `universe_pit.py`) — the highest-leverage open item, unlocks the reasoning for CS-02/03/04/05/09 too. (2) `amihud_illiquidity`: Tier A cross-sectional translation (needs a per-symbol "in today's top-illiquidity-quintile" flag, same shape as `AtrVolPremium`, precomputed from the existing factor-zoo universe panel). (3) `atr_normalized` + `amihud_illiquidity`: mechanism + confidence-multiplier scoring (Chapter 4 clauses 1/2) before a formal proposal. (4) `factor-zoo-v1` still deferred from Strategy Management, pending a clearer explanation. (5) Earnings-dates or SEC EDGAR fundamentals data layer — neither started, [data-layers.md](data-layers.md). (6) ADR 0008 implementation — blocked on an Alpaca account. |
