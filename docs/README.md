...
| Current state | Research reorganized into a chaptered, living program — see [research-program.md](research-program.md). Chapters 1-3 (single-asset/TA+calendar, cross-sectional, macro/event-driven) hold the existing strict falsification standard unchanged; a new, parallel Chapter 4 (risk-budgeted ensemble construction, [ADR 0007](adr/0007-risk-budgeted-ensemble-acceptance.md), accepted) answers a different question — not "is this proven," but "is this worth a small, sized, loss-controlled bet" — reusing ADR 0004's existing loss-based sizing formula and drawdown halt rather than inventing new risk machinery |
| Verification | `367 passed` at `0.71.0` |
| Completed | Stage 8; Stage 9A Cycles 1–6; CTA v1/v2; TA Breakout, Wave Pull, ETF-12 rotation, both calendar candidates, Overnight Gap, Fed put yield-stress precursor v1+v2+v3 — all closed and indexed in [research-protocols/README.md](research-protocols/README.md); labor-market claims lead-lag operationalized and bounded-explored (not yet scored); Chapter 4's first full pass (CTA v2, Wave Pull, Calendar Day-of-Week) is now closed with a correlation-aware significance test, not an approximation |
| Next product work | No queued Stage 9A candidate. All three Chapter 4 candidates scored so far have been adversarially checked and none currently has a settled positive read (see Active research) — building the ensemble-construction engine or deciding the minimum-breadth floor would be sizing infrastructure around signals that have not yet earned it, so neither is the right next step. Chapter 1-3 open threads (not yet sections): Dow theory swing structure, Fibonacci, golden/death cross, 52-week-high anchoring (Ch.1); the six Tier-4-blocked cross-sectional ideas (Ch.2); the 7-variable macro library, PEAD/earnings-gap (needs an earnings-date ingestion module — `yfinance.get_earnings_dates()` confirmed working, free, 1987-2026 depth), policy-exposure factor (Ch.3) — see [research-program.md](research-program.md) for the full inventory |
| Active research | Chapter 4 (ADR 0007, accepted): 3 candidates scored, none currently has a settled, adversarially-checked positive read. CTA v2 cleanly not eligible. Wave Pull's `TLT`-only "eligible" report was a winner's-curse artifact — symmetric 12-asset rescoring (`2/11`, `GLD`+`TLT`) is statistically indistinguishable from the calibrated chance rate (`P(X≥2)≈65%`); walked back to "clean candidate for Chapters 1-3," not "Chapter 4-eligible." Calendar Day-of-Week's `6/12` was directly tested with a correlation-aware joint null (not an approximation): `p≈0.13`-`0.14`, stable across three independent block-size checks — also not distinguishable from chance. Orthogonality measured across all 8 nominally-eligible signal-slots (3 of 28 pairs redundant, all 3 among Day-of-Week's winners) and the full 12-asset Day-of-Week correlation matrix (`31`/`66` pairs redundant — the broader universe is saturated with ordinary equity-beta correlation). Stage 9B remains gated |
