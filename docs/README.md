...
| Current state | Eight Stage 9A protocols closed `not_material_or_not_consistent`, each a different shape (confound, power limit, weak test design, clean-but-null, clean-and-decisive-null, well-powered-null-outside-the-technical-pattern-family, well-powered-directionally-consistent-but-uncorrected-null, and now an eighth: the session's most decisive negative — every asset opposite-signed from the hypothesis); Cycle 5 is closed in full, all three candidates executed. `0.54.0` is an infrastructure fix (audit H2) between research cycles, not a new one. |
| Verification | `284 passed` at `0.54.0` |
| Completed | Stage 8; Stage 9A Cycles 1–5 each selected, locked, and executed to a closed result; TA Breakout v1, Wave Pull v1, ETF-12 cross-sectional rotation v1, Calendar Turn-of-Month v1, Calendar Day-of-Week v1, and Overnight Gap Continuation v1 locked, executed, and closed; `Wave Pull`'s crash bug fixed and regression-tested; an independent, adversarially verified next-priority evaluation (2026-08-20) scored five options and surfaced Cycle 5; Overnight Gap Continuation v1's new joint-paired-resampling design passed an independent adversarial pre-lock code review (six issues found and fixed) before any data was touched |
| Next product work | No queued research task. CTA v2 remains parked pending a pooled-portfolio engine (cost corrected 2026-08-20: adjacent live-portfolio infrastructure exists but isn't directly reusable) — its overlap concern with rotation is now moot, since rotation ran and found nothing. `S/R Bounce` needs a materially different construction to re-score, and a genuinely distinct one (round-number levels) is now known but blocked on an adjusted-vs-nominal-price data gap (ADR 0002). Fed put is documented and blocked on ADR 0006 plus two free-but-unbuilt data sources. Overnight Gap Continuation v1's disclosed diagnostic suggests a reversal-shaped (not continuation) pattern that this protocol could not test — a reversal-framed candidate would be new, independently justified work. MACD and full Elliott Wave counting were assessed and not recommended — see [pending checklist](brainstorm/2026-08-19-pending-candidate-checklist.md) |
| Active research | None active; Cycle 5 is closed in full and the cheap tier of the checklist is exhausted (eight closed results) — the next step is a deliberate choice (a reversal-framed follow-up, CTA v2's engine, macro data investment, or a fresh cycle), not a default; Stage 9B remains gated |
...
Local-optimum guard: Stage 8 is closed. Do not add more infrastructure or UX polish unless a concrete defect blocks research. Cycle 1 is closed `not_evaluable`; Cycle 2, Cycle 3, TA Breakout v1, Wave Pull v1, ETF-12 cross-sectional rotation v1, Calendar Turn-of-Month v1, Calendar Day-of-Week v1, and Overnight Gap Continuation v1 are all closed `not_material_or_not_consistent`, each for a different reason (confound, power limitation, weak event/placebo separation, clean-separation-but-null, clean decisive null, well-powered null in the first non-technical-pattern mechanism tested, well-powered directionally-consistent-but-uncorrected null, and now the session's most decisive negative — every asset opposite-signed from the hypothesis — read each result's own reading before assuming they generalize to each other). None licenses a reflexive follow-up — do not widen a window/threshold/tolerance/horizon grid, add a pooled version, or otherwise retry any same claim in reaction to its result; any such attempt is a new, independently justified candidate, not a repair. Cycle 5 is closed in full; the cheap tier of the pending checklist is exhausted (eight closed results); the next step (a reversal-framed follow-up to overnight-gap, CTA v2's engine, data investment for Fed put, or a fresh cycle) is a deliberate choice for the user, not a default.
