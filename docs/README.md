...
| Current state | [Chaptered research program](research-program.md). **Chapter 2 §3-4 closed**: cross-sectional momentum v1 (CS-01) and sector rotation v1, both `not_material_or_not_consistent` — see [CS-01](research-results/cross-sectional-momentum-v1.md) and [sector rotation](research-results/sector-rotation-v1.md) results. Sector rotation directly tested the user's own named observation ("long chips, short software") at the aggregate GICS level — a clean null (correlation slightly negative), unlike CS-01's borderline case. A real, separate, deeper survivorship bias was also named and disclosed (index membership is itself endogenous to size/performance — likely a real Tier 4 vendor-data case if ever pursued, see [the idea library](brainstorm/2026-08-20-cross-sectional-experiment-ideas.md)). **[ADR 0010 (accepted)](adr/0010-long-short-ensemble-construction.md)** + **[ensemble-construction-engine-v1.md](ensemble-construction-engine-v1.md)**: amends ADR 0004's account model, gives Chapter 4's ensemble-construction engine an exact, disciplined design (formulas, function signatures, worked example) — not yet implemented. Chapter 4 (ADR 0007) remains the primary actionable line — 2 live candidates, `atr_normalized` (Tier A execution, "ATR Vol Premium") and `amihud_illiquidity` (next for the same treatment). Chapters 1/3 deliberately paused. Chapter 5 ([ADR 0008](adr/0008-bounded-paper-trading.md)) accepted, not built. Product: Today/Symbol Research/Strategy Lab/Strategy Management unified across Tier A/B ([strategy-library.md](strategy-library.md)); Macro page carries a labeled, untested "equity read" hypothesis per event ([ADR 0006](adr/0006-macro-data-contract.md) clause 2 exception); sitewide footer links the chaptered research program. |
| Verification | `432 passed` at `0.84.0` |
| Next product work | (1) **ADR 0010**: implement [ensemble-construction-engine-v1.md](ensemble-construction-engine-v1.md)'s exact design (`composite_alpha_scores`, `shrinkage_covariance`, `construct_portfolio` in a new `backend/app/ensemble.py`) against its own test/acceptance checklist — real, new engineering, not yet started. (2) **Chapter 2**: CS-02/03/04/05/09, a finer GICS Sub-Industry-level rotation test (e.g. "Semiconductors" specifically), or a market-neutral/beta-adjusted re-design of CS-01 — each needs its own hypothesis-engineering note, Stage 9A score, and preregistration. (3) `amihud_illiquidity`: Tier A cross-sectional translation (per-symbol "in today's top-illiquidity-quintile" flag, same shape as `AtrVolPremium`). (4) `atr_normalized` + `amihud_illiquidity`: mechanism + confidence-multiplier scoring (Chapter 4 clauses 1/2) before a formal proposal. (5) `factor-zoo-v1` still deferred from Strategy Management, pending a clearer explanation. (6) Earnings-dates or SEC EDGAR fundamentals data layer — neither started, [data-layers.md](data-layers.md). (7) ADR 0008 implementation — blocked on an Alpaca account. |
