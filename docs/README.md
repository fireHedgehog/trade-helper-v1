...
| Current state | [Chaptered research program](research-program.md) — see CHANGELOG for detail per version. **Chapter 4 is the default evaluation for any new candidate** (hard rule, [identity.md](identity.md)). **Three genuinely independent live candidates** (`|r|<0.5` all pairs): `amihud_illiquidity` (confidence multiplier `0.33`), `max_effect` (`0.56`), `expected_skewness_proxy` (`0.81`, strongest yet) — [academic anomalies result](research-results/academic-anomalies-chapter4-v1.md). **Ensemble-construction engine** (`backend/app/ensemble.py`, [ADR 0010](adr/0010-long-short-ensemble-construction.md) + [design doc](ensemble-construction-engine-v1.md)) proven at real 3-signal breadth — [result](research-results/ensemble-smoke-test-v2.md): `100.00%` gross, `~0%` net, all three signals contributing. `atr_normalized`'s cross-sectional form does **not** survive point-in-time correction (own-history "ATR Vol Premium" Tier A strategy is a different, unaffected claim). Sector rotation and `corwin_schultz_spread` closed as decisive negatives. Chapters 1/3 paused. Chapter 5 ([ADR 0008](adr/0008-bounded-paper-trading.md)) accepted, not built. |
| Verification | `444 passed` at `0.88.0` |
| Next product work | (1) Clause 1 (mechanism) + clause 2 (EV + uncertainty band, already computed) for all three live candidates before a formal Chapter 4 proposal. (2) Sector/cluster caps (ADR 0010 §5) unimplemented in `ensemble.py` — needed before any real deployment. (3) A flipped-direction `corwin_schultz_spread` (buy liquid, short illiquid) is a plausible new, independently-justified candidate, not yet tried. (4) Chapter 2 non-sector ideas (CS-02/03/04/05/09) if picked back up. (5) `factor-zoo-v1` still deferred from Strategy Management. (6) Earnings-dates or SEC EDGAR fundamentals data layer — neither started, [data-layers.md](data-layers.md). (7) ADR 0008 implementation — blocked on an Alpaca account. |
