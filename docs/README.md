...
| Verification | `391 passed` at `0.75.1` (docs-only restructure, no test change) |
...
| Next product work | Three threads. (1) Factor zoo follow-up: propose the least-redundant survivors (`alpha028`/`004`/`026`, `atr_normalized`) as individual Chapter 4 candidates with a stated mechanism each, run a transaction-cost-sensitivity check on the whole reversal cluster before trusting its Sharpe numbers, and check `atr_normalized`'s regime concentration (ADR 0007 clause 5) — see [research-program.md](research-program.md) Chapter 4 §5. (2) Data layer follow-up, only when it's actually needed: earnings-date ingestion (confirmed-free `yfinance.get_earnings_dates()`, unblocks PEAD) and/or SEC EDGAR PIT-fundamentals ETL (confirmed-free but real work, unblocks value/quality factors) — see [data-layers.md](data-layers.md), neither started. (3) ADR 0008 implementation — `live_price_snapshots`, `paper_ledger_events`, `research_catalog.py` corrections, and the Alpaca integration module are all still unbuilt, and real connectivity is blocked on the user creating an Alpaca paper account and API keys; schema/metrics work can proceed without that. Chapter 1-3 stays intentionally paused — see Current state — with its open threads (Dow theory, Fibonacci, golden/death cross, the six Tier-4-blocked cross-sectional ideas, the macro library, PEAD, policy-exposure factor) unchanged and unabandoned in [research-program.md](research-program.md) |
