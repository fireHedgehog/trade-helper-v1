...
Every §1–3 candidate has been adversarially checked and has failed to hold up — the calibration discipline (§4) is catching its own candidates under a properly rigorous test, exactly its intended function. The ensemble-construction engine and minimum-breadth floor stay unbuilt (ADR 0007 Consequences), pending a settled positive candidate.

**Next step**: two live candidates, `atr_normalized` (§5/§5b/§5c cleared) and `amihud_illiquidity` (§5d, independent, cost-checked, regime concentration not yet run). Both still need clause 1 (a written economic mechanism) and clause 2 (a cross-validated point estimate with an explicit uncertainty band) before a formal Chapter 4 proposal. `alpha028`/`alpha004`/`alpha026`/`low_volatility`/`max_effect` are answered — redundant or immaterial after cost — and closed.
