...
Every §1–3 candidate has been adversarially checked and has failed to hold up — the calibration discipline (§4) is catching its own candidates under a properly rigorous test, exactly its intended function. §6/§10 is the ensemble-construction engine ADR 0007 named and deferred: designed, then implemented and smoke-tested the same day. §7-9 are the Sharpe/CAGR/drawdown-based evaluation this chapter always intended for a modest, uncertain edge — not a falsification p-value — applied directly per user feedback (`2026-08-21`) that Chapter 2's bar had been misapplied to Chapter 4-shaped claims, and that a strategy running live in the app without any documented performance survey is its own kind of gap.

**Next step**: `amihud_illiquidity` (§9) is the strongest live candidate — positive Sharpe, positive CI lower bound, survives both the point-in-time fix and real cost. `atr_normalized`'s own-history Tier A execution (§5/§5b/§5c/§8) remains live and documented, but its *cross-sectional* form is now a disclosed non-candidate (§10) — the ensemble's real second signal still needs to be found (a different cross-sectional factor, or a finer/differently-scoped version of an existing one), not assumed to already exist. Sector rotation (§4/§7) is a converging double-null and not currently pursued further at this design. `amihud_illiquidity` still needs clause 1 (a written economic mechanism — the Amihud 2002 illiquidity-premium literature already supplies one, needs stating for this project's own record) and clause 2 (§9's numbers are the raw material) before a formal Chapter 4 proposal. `alpha028`/`alpha004`/`alpha026`/`low_volatility`/`max_effect` are answered — redundant or immaterial after cost — and closed.
