...
Every §1–3 candidate has been adversarially checked and has failed to hold up — the calibration discipline (§4) is catching its own candidates under a properly rigorous test, exactly its intended function. §6 is the design for the ensemble-construction engine and minimum-breadth floor ADR 0007 named and deferred; it is accepted but not yet implemented. §7 and §8 are the Sharpe/CAGR/drawdown-based evaluation this chapter always intended for a modest, uncertain edge — not a falsification p-value — applied directly per user feedback (`2026-08-21`) that Chapter 2's bar had been misapplied to a Chapter 4-shaped claim, and that a strategy running live in the app without any documented performance survey is its own kind of gap.

**Next step**: three live candidates — `atr_normalized` (§5/§5b/§5c/§8 all real, all documented), `amihud_illiquidity` (§5d, independent, cost-checked, regime concentration not yet run), and sector rotation (§4/§7, converging nulls under two different statistics). All three still need clause 1 (a written economic mechanism) and clause 2 (a cross-validated point estimate with an explicit uncertainty band, §7/§8's own numbers being the raw material for that) before a formal Chapter 4 proposal. `alpha028`/`alpha004`/`alpha026`/`low_volatility`/`max_effect` are answered — redundant or immaterial after cost — and closed.
