...
| Options (IV, Greeks, open interest by strike) | not built | **researched, not purchased** | cheapest real option (`EODHD`) is `Q4 2023`-present only, not true history | not free — cheapest researched `2026-08-21`: [EODHD US options API](https://eodhd.com/lp/us-stock-options-api), `$99.99/mo`; deeper history (OptionMetrics IvyDB, CBOE DataShop) has no public price, real four-figure/year territory | 6,000+ US stocks at EODHD, <3yr depth | none yet — no queued hypothesis needs it; "call wall/put wall" is a derived metric from OI-by-strike, not a separate purchase |
| Level 2 / order-book depth | **excluded by design, not planned** | n/a | n/a | researched `2026-08-21` (Databento, metered/pay-per-byte, cheapest access model found) but not a candidate purchase | n/a | never — same reasoning as the Intraday/tick row below: this project never reaches live/HFT execution, so microstructure-priced data buys nothing it would use |
