"""Run the Overnight Gap Continuation v1 gap-conditioned signed
forward-return experiment.

Usage (from backend/):
    python -m app.run_overnight_gap_continuation

Implements docs/research-protocols/overnight-gap-continuation-v1.md. This is
a no-trade event study and significance test; it may output only
...
from .research import holm_adjust, overnight_gap_bootstrap
from .run_experiment import _atomic_json
from .store import load_bars

ROOT = Path(__file__).parents[2]
SPEC_PATH = ROOT / "research/experiments/overnight-gap-continuation-v1.json"
SPEC_SHA256 = "8cf8881c155fc7006b76055c443a3e830e214ca9d2f65d2d05cc350a68df17e5"
...
def run_bootstrap(spec: dict, bars_by_symbol: dict) -> dict:
    ...
    for symbol in order:
        ...
        per_asset[symbol] = overnight_gap_bootstrap(
            opens,
            closes,
            quantile=spec["event_and_placebo"]["quantile"],
            ...
            cooldown=spec["event_and_placebo"]["cooldown_sessions"],
            horizon=spec["signed_forward_return"]["horizon_sessions"],
...
def evaluate_gates(spec: dict, per_asset: dict) -> dict:
    ...
    materiality_min = float(spec["gates"]["materiality"]["mean_signed_forward_return_minimum"])
    order = sorted(spec["universe"])
    ...
    for symbol in order:
        ...
        if result["insufficient_events"]:
            ...
            continue
        event_material = (
            result["observed_gap_mean_signed_forward_return"] >= materiality_min
            and result["holm_p_event"] is not None
            and result["holm_p_event"] <= alpha
        )
        beats_placebo = (
            result["observed_gap_mean_signed_forward_return"]
            > result["observed_placebo_mean_signed_forward_return"]
            and result["p_gap_vs_placebo"] is not None
            and result["p_gap_vs_placebo"] <= alpha
        )
        materiality_by_asset[symbol] = {
            "eligible": True,
...
def main() -> None:
    ...
    output = ROOT / "output/research/overnight-gap-continuation-v1" / SPEC_SHA256
    _atomic_json(
