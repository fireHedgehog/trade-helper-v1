"""Run Fed put: yield-stress precursor v1.

Usage (from backend/):
    python -m app.run_fed_put_yield_stress_precursor

Implements docs/research-protocols/fed-put-yield-stress-precursor-v1.md.
Thesis Track evidence-layer output only (not_evaluable / weak_evidence /
evidence_present) -- no trade, no Stage 9B authorization, no signal.
"""
...
SPEC_PATH = ROOT / "research/experiments/fed-put-yield-stress-precursor-v1.json"
SPEC_SHA256 = "61eb422ef52275bb0947c8fe932f742e205c1aaa57fe665713734c6d9a06623f"
...
def load_aligned_yields(spec: dict) -> tuple[dict, list[str]]:
    """DGS2/DGS10 bars restricted to their true common date intersection."""
...
def main() -> None:
    ...
    # Truncate the unusable NaN-lookback prefix so placebo windows never draw it.
    offset = lookback
    ...
    episode_windows = []  # (start, end) in truncated-array space, precursor window only
    excluded_ranges = []  # (start, end) in truncated-array space, precursor window + buffer
    episode_records = []
    for episode in spec["episodes"]:
        start_idx = _date_index(dates, episode["start"]) - offset
        end_idx = _date_index(dates, episode["end"]) - offset
        ...
        episode_records.append({"name": episode["name"], "start": episode["start"], "end": episode["end"]})
    ...
    output = ROOT / "output/research/fed-put-yield-stress-precursor-v1" / SPEC_SHA256
    _atomic_json(
