"""Run the Calendar Day-of-Week v1 Monday daily-return-differential experiment.

Usage (from backend/):
    python -m app.run_calendar_day_of_week

Implements docs/research-protocols/calendar-day-of-week-v1.md. This is a
...
from .research import dow_bootstrap, holm_adjust
from .run_experiment import _atomic_json
from .store import load_bars

ROOT = Path(__file__).parents[2]
SPEC_PATH = ROOT / "research/experiments/calendar-day-of-week-v1.json"
SPEC_SHA256 = "8283d00b5a10a7778fd5826a6931226013c3435b4f5f14fbe07c40a868d7ff19"
...
def run_bootstrap(spec: dict, bars_by_symbol: dict) -> dict:
    ...
    for symbol in order:
        ...
        dates = bars["date"]
        per_asset[symbol] = dow_bootstrap(
            closes,
            dates,
...
def evaluate_gates(spec: dict, per_asset: dict) -> dict:
    ...
    materiality_max = float(spec["gates"]["materiality"]["daily_differential_maximum"])
    order = sorted(spec["universe"])
    ...
    for symbol in order:
        ...
        if result["insufficient_events"]:
            ...
            continue
        event_material = (
            result["observed_daily_differential"] <= materiality_max
...
def main() -> None:
    ...
    output = ROOT / "output/research/calendar-day-of-week-v1" / SPEC_SHA256
    _atomic_json(
