"""Run the TA Breakout v1 rejected-resistance vs. raw-breakout-placebo experiment.

Usage (from backend/):
    python -m app.run_ta_breakout

Implements docs/research-protocols/ta-breakout-v1.md. This is a no-trade
event study and significance test; it may output only material_and_consistent
/ not_material_or_not_consistent / invalid. No cost, execution, or portfolio
simulation is authorised by this run.
"""
...
from .research import holm_adjust, ta_breakout_bootstrap
from .run_experiment import _atomic_json
from .store import load_bars

ROOT = Path(__file__).parents[2]
SPEC_PATH = ROOT / "research/experiments/ta-breakout-v1.json"
SPEC_SHA256 = "40929c9de93633d15284ded96b6dde84998932044ef502ef75954dbb375bda6a"
...
def run_bootstrap(spec: dict, bars_by_symbol: dict) -> dict:
    ...
    for symbol in order:
        ...
        per_asset[symbol] = ta_breakout_bootstrap(
            closes,
...
def main() -> None:
    ...
    output = ROOT / "output/research/ta-breakout-v1" / SPEC_SHA256
    _atomic_json(
